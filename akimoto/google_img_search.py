...
def scrapingImages(name: str, dir_name: str, max_num: int, offset: int):
    google_crawler = GoogleImageCrawler(storage={'root_dir': f'./imgs/{dir_name}'})
    google_crawler.crawl(keyword=f'{name}', max_num=max_num, file_idx_offset=offset)
    # yahoo_crawler = BingImageCrawler(storage={'root_dir': f'./imgs/{dir_name}'})
    # yahoo_crawler.crawl(keyword=f'{name}', max_num=max_num, file_idx_offset=offset+max_num)
